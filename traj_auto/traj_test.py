...
from pcp_tools import Scene
from pcp_tools.gaussian_render import render
from pcp_tools.gaussian_model import GaussianModel
from pcp_tools.arguments import ModelParams, PipelineParams, get_combined_args
...
def render_trajectory(dataset: ModelParams, iteration: int, pipeline: PipelineParams,
    ...
    with torch.no_grad():
        ...
        for i, camera_pose in enumerate(trajectory):
            ...
            view.cal_transform()
            # torch.cuda.synchronize()会阻塞当前线程，直到所有在调用该函数之前发出的CUDA核心调用完成为止。
            # 这包括CUDA核心上所有计算、传输和内存操作的完成。该函数可以用于确保在进行时间敏感的操作之前，之前的计算已经完成，以避免并发问题。
            # 通常情况下，当你需要确保在检查或使用由GPU计算得到的结果之前，先等待之前的计算完成时，可以使用torch.cuda.synchronize()函数。
...
if __name__ == "__main__":
    ...
    parser.add_argument("--anti_alias", action="store_true", default=False)  # 奇异点滤除
