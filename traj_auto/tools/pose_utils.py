...
def generate_ellipse_path(views, n_frames=600, const_speed=True, z_variation=0., z_phase=0.):
    ...
    ratio = 0.5  # use to control the distance from views to sense center
    low = -ratio * sc + offset
    high = ratio * sc + offset
    # Optional height variation need not be symmetric
    z_low = np.percentile((poses[:, :3, 3]), 10, axis=0) + 0.5
    z_high = np.percentile((poses[:, :3, 3]), 90, axis=0)

    def get_positions(theta, is_ellipse_3d=True):
        # Interpolate between bounds with trig functions to get ellipse in x-y.
        # Optionally also interpolate in z to change camera height along path.
        if is_ellipse_3d:  # 椭球路线
            return np.stack([
            ...
            zList = np.ones(theta.shape, theta.dtype)
            return np.stack([
                (low[0] + (high - low)[0] * (np.cos(theta) * .5 + .5)),
                (low[1] + (high - low)[1] * (np.sin(theta) * .5 + .5)),
                z_variation * (z_low[2] + ((z_high - z_low)[2] / 2) * zList),
            ], -1)

    theta = np.linspace(0, 2. * np.pi, n_frames + 1, endpoint=True)
    positions = get_positions(theta, False)
