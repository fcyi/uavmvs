...
from general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation, strip_symmetric, build_scaling_rotation
...
from graphics_utils import BasicPointCloud, getWorld2View
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        if self.multi_occ:
            occ_multiplier = inverse_sigmoid(
                0.99 * torch.ones((len(fused_point_cloud), self.n_lvl_occ, 1), dtype=torch.float, device="cuda"))
        else:
        ...
        self.base_gaussian_mask = torch.zeros((self.get_xyz.shape[0]), device="cuda", dtype=torch.bool,
                                              requires_grad=False)
        self.target_reso_lvl = torch.zeros((self.get_xyz.shape[0]), device="cuda", dtype=torch.long,
                                           requires_grad=False)
    ...
    def load_ply(self, path):
        ...
        self._opacity = nn.Parameter(
            torch.tensor(opacities.copy(), dtype=torch.float, device="cuda").requires_grad_(True))
        self._occ_multiplier = nn.Parameter(
            torch.tensor(occ_multiplier.copy(), dtype=torch.float, device="cuda").requires_grad_(self.multi_occ))
        self._dc_delta = nn.Parameter(
            torch.tensor(dc_delta.copy(), dtype=torch.float, device="cuda").requires_grad_(self.multi_dc))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,
        ...
        self.xyz_gradient_accum = torch.cat(
            [self.xyz_gradient_accum, torch.zeros((len(new_xyz), self.reso_lvls, 1), device="cuda")], dim=0)
    ...
    def insert_large_gaussians(self, mask, cur_min_pixel_sizes, reso_lvl, scene_extent):
        ...
        voxel_feature_dc = voxel_pooling(rel_pos, self._features_dc.reshape([N, -1])[mask].cpu(),
                                         voxel_reso).pooled_features.cuda()
        voxel_feature_rest = voxel_pooling(rel_pos, self._features_rest.reshape([N, -1])[mask].cpu(),
                                           voxel_reso).pooled_features.cuda()
        voxel_opacity = voxel_pooling(rel_pos, self._opacity.reshape([N, -1])[mask].cpu(),
                                      voxel_reso).pooled_features.cuda()
        voxel_occ_multiplier = voxel_pooling(rel_pos, self._occ_multiplier.reshape([N, -1])[mask].cpu(),
                                             voxel_reso).pooled_features.cuda()
        voxel_dc_delta = voxel_pooling(rel_pos, self._dc_delta.reshape([N, -1])[mask].cpu(),
                                       voxel_reso).pooled_features.cuda()
        voxel_rotation = voxel_pooling(rel_pos, self._rotation.reshape([N, -1])[mask].cpu(),
                                       voxel_reso).pooled_features.cuda()
        voxel_max_pixel_sizes = voxel_pooling(rel_pos, self.max_pixel_sizes.reshape([N, -1])[mask].cpu(),
                                              voxel_reso).pooled_features.cuda()
        voxel_min_pixel_sizes = voxel_pooling(rel_pos, self.min_pixel_sizes.reshape([N, -1])[mask].cpu(),
                                              voxel_reso).pooled_features.cuda()
        voxel_scaling = voxel_pooling(rel_pos, self._scaling.reshape([N, -1])[mask].cpu(),
                                      voxel_reso).pooled_features.cuda()
        voxel_cur_min_pixel_sizes = voxel_pooling(rel_pos, cur_min_pixel_sizes.reshape([N, -1])[mask].cpu(),
                                                  voxel_reso).pooled_features.cuda()
